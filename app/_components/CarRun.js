...
for (let run = 10000; run <= 1000000; run += 10000) {
  carManufacturers.push({ label: run.toString(), value: run });
}

export default function CarRun({ filters, setFilters }) {
  ...
  return (
    <div className="relative w-48" ref={dropdownRef}>
      ...
      >
        <span
          className={`ml-11 mb-1 absolute inset-0 flex items-center justify-center text-gray-200 text-xl font-bold pointer-events-none transition-opacity duration-200 ${
            selectedItem || hoveredItem ? "opacity-100" : "opacity-0"
          }`}
        >
          {hoveredItem || selectedItem ? "Run(km)" : ""}
        </span>
        {selectedItem ? selectedItem.label + "+" : "Run(km)"}
